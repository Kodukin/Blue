...
@Component({
  ...
  imports: [
    ...
    NgFor,
    SubmitComponent,
    CheckAvailability2Component
],
  templateUrl: './book-now2.component.html',
  styleUrls: ['./book-now2.component.css'],
})
export class BookNow2Component implements AfterViewInit, OnInit {
  ...
  ngOnInit() {
    this.bookingService
      ...
      .subscribe({
        next: (response) => {
          this.bookingSpace = [...response].reverse();
        },
        error: (error) => {
          console.log(error);
        },
...
onChange: (selectedDate, dateStr) => {
  ...
  submit() {
    ...
    this.bookingService
      .checkAvailability({
        end: this.checkForm.value.end,
        start: this.checkForm.value.start,
        hall: this.checkForm.value.space,
      })
      .pipe(
        map((response) => {
            ...
            for (let key1 in response[key]) {
              if (key1 === 'status') {
                if (response[key][key1] === 'available') {
                  ...
                  this.bookingMessage =
                    'Hall is not available, please choose/book another day';
                  setTimeout(() => {
                    this.bookingMessage = '';
                  }, 3000);
                  this.availabilityStatus = false;
    ...
    this.notifyLoaderService.complete.subscribe((data: handler) => {
      ...
      setTimeout(() => {
        ...
        if (data.success === true) {
          this.showToast(this.successMsg);
        } else {
  ...
  successMsg =
    '<i class="fa-solid fa-circle-check"></i>Booking made successfully';
  errorMsg =
    '<i class="fa-solid fa-circle-xmark"></i> Error occurred, try again';
  invalidMsg =
